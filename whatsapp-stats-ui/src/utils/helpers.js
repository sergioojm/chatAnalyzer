...
export function downloadPdfReport(stats){
  ...
  const now = new Date();
  doc.setFontSize(9);
  doc.text(`Generado: ${now.toLocaleString()}`, 40, 64);
  doc.setFontSize(11);
  ...
  // Summary table
  const summaryBody = [
    ["Rango", `${input.from ?? "-"} → ${input.to ?? "-"}`],
    ["Mensajes parseados", String(totals.totalMessagesParsed ?? 0)],
    ["Mensajes analizados", String(totals.totalMessagesSelected ?? 0)],
    ["Participantes (seleccionados)", String(totals.totalParticipantsSelected ?? 0)],
    ["Palabras analizadas", String(totals.totalWordsSelected ?? 0)],
    ["Días analizados", String(global?.messagesPerDay?.uniqueDays ?? 0)],
    ["Promedio mensajes/día", (global?.messagesPerDay?.avgPerDay ?? 0).toFixed(2)],
    ["Hora más activa", global?.busiest?.hour != null ? `${String(global.busiest.hour).padStart(2,"0")}:00 (${fmt(global.busiest.hourMessages)} msgs)` : "-"],
    ["Día más activo", global?.busiest?.weekdayName ? `${global.busiest.weekdayName} (${fmt(global.busiest.weekdayMessages)} msgs)` : "-"]
  ];
  autoTable(doc, {
    startY: 80,
    head: [["Campo","Valor"]],
  ...
  // Global top words
  autoTable(doc, {
    startY: doc.lastAutoTable ? doc.lastAutoTable.finalY + 12 : 150,
  ...
  // Top participants (top 10 by mensajes)
  const topParticipants = [...participants].sort((a,b)=>(b.messagesAnalyzed ?? 0) - (a.messagesAnalyzed ?? 0)).slice(0,10);
  autoTable(doc, {
    startY: doc.lastAutoTable ? doc.lastAutoTable.finalY + 12 : undefined,
    head:[["Top participantes","Mensajes","Sent. media","Pos%","Neu%","Neg%","Palabras","Únicas","Pal/msg"]],
    body: topParticipants.map(p=>[
      p.author ?? "-",
  ...
  // Full participants table if space allows (append afterwards)
  if ((participants ?? []).length > 10) {
    const remaining = participants.slice(10);
    autoTable(doc, {
      startY: doc.lastAutoTable ? doc.lastAutoTable.finalY + 12 : undefined,
      head:[["Otros participantes","Mensajes","Sent. media","Pos%","Neu%","Neg%"]],
      body: remaining.map(p=>[
        p.author ?? "-",
        String(p.messagesAnalyzed ?? 0),
        (p.sentimentAvg ?? 0).toFixed(3),
        String(p.percent?.positive ?? 0),
